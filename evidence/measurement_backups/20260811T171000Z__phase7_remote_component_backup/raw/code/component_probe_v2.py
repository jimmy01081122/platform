...
def make_attention_points(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for role, contexts in (("fit", [128, 2048, 8192, 16384]), ("held-out", [512, 4096, 12288, 28672])):
        for context in contexts:
            points.append({"id": f"CMP-A0-{context}" if role == "fit" else f"CMP-A1-{context}", "family": "CMP-A", "role": role, "mode": "prefill", "context_tokens": context, "active_sequences": 1})
    for role, rows in (("serving-fit", [(512, 1), (512, 4), (4096, 1), (4096, 4), (16384, 1), (16384, 4)]), ("serving-held-out", [(2048, 2), (2048, 8), (8192, 2), (8192, 8), (28672, 2), (28672, 8)])):
        prefix = "CMP-A2" if role == "serving-fit" else "CMP-A3"
        for context, batch in rows:
            points.append({"id": f"{prefix}-{context}x{batch}", "family": "CMP-A", "role": role, "mode": "decode", "context_tokens": context, "active_sequences": batch})
    return points
...
def main() -> int:
    ...
    write_json(args.output_dir / "manifest.json", {
        "schema_version": "phase7-component-probe-v1",
        "status": "RUNNING",
    ...
    if "A" in groups:
        ...
        for point in make_attention_points(config):
            batch = int(point["active_sequences"])
            context = int(point["context_tokens"])
            if point["mode"] == "prefill":
                generator = torch.Generator(device=device).manual_seed(1000 + context)
                x = torch.randn((batch, context, int(config["hidden_size"])), device=device, dtype=torch.bfloat16, generator=generator)
                fn = lambda x=x: attention_call(x, attention["q"], attention["k"], attention["v"], attention["o"], num_heads, num_kv_heads, head_dim, True)
                point = {**point, "input_bytes": int(x.numel() * 2), "weight_bytes": sum(int(value.numel() * 2) for value in attention.values()), "kernel_identity_source": "torch.nn.functional.scaled_dot_product_attention + checkpoint self_attn q/k/v/o layer0"}
            else:
                generator = torch.Generator(device=device).manual_seed(2000 + context * 10 + batch)
                cache_hidden = torch.randn((batch, context, int(config["hidden_size"])), device=device, dtype=torch.bfloat16, generator=generator)
                query = torch.randn((batch, 1, int(config["hidden_size"])), device=device, dtype=torch.bfloat16, generator=generator)
                with torch.inference_mode():
                    k_cache = torch.matmul(cache_hidden, attention["k"].t()).view(batch, context, num_kv_heads, head_dim).transpose(1, 2).contiguous()
                    v_cache = torch.matmul(cache_hidden, attention["v"].t()).view(batch, context, num_kv_heads, head_dim).transpose(1, 2).contiguous()
                fn = lambda query=query, k_cache=k_cache, v_cache=v_cache: decode_attention_call(query, k_cache, v_cache, attention["q"], attention["o"], num_heads, num_kv_heads, head_dim)
                point = {**point, "query_input_bytes": int(query.numel() * 2), "kv_cache_bytes": int((k_cache.numel() + v_cache.numel()) * 2), "weight_bytes": int(attention["q"].numel() * 2 + attention["o"].numel() * 2), "kernel_identity_source": "torch.nn.functional.scaled_dot_product_attention decode + checkpoint self_attn q/o layer0"}
            with torch.inference_mode():
                measure_point(point, fn, args.output_dir, args.repetitions, args.max_repetitions)
            if point["mode"] == "prefill":
                del x
            else:
                del query, k_cache, v_cache, cache_hidden
