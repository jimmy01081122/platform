...
def main() -> int:
    ...
    for layer in range(int(config["num_hidden_layers"])):
        for expert in range(int(config["num_local_experts"])):
            ...
            catalog[str(object_id)] = [{"layer_id": layer, "local_expert_id": expert, "global_object_id": object_id, "tensor_name": source["name"].split(f".experts.{expert}.", 1)[1], **source} for source in object_sources(sources, layer, expert)]
