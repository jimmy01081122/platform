...
def run_one_request(
    ...
    if prompt_token_ids is not None:
        ...
        input_count = None
    sampler = TelemetrySampler(run_dir / "telemetry.jsonl") if runtime_class == "TELEMETRY" else None
    ...
    try:
        ...
        if runtime_class == "KERNEL_PROFILE":
            results, profiler_info = profiler_run(
                torch_module,
                invoke,
                run_dir / "profiler" / f"{logical_id}.json",
                run_dir / "profiler" / f"{logical_id}.table.txt",
            )
        else:
            results = invoke()
    finally:
...
def main() -> int:
    ...
    try:
        ...
        if args.input_tokens is None and not args.prompt_text:
            raise CampaignFailure("provide --input-tokens or --prompt-text")
        if args.input_tokens is not None and args.prompt_text:
            raise CampaignFailure("do not combine --input-tokens and --prompt-text")
        ...
        print(json.dumps({"status": "PASS", "run_dir": str(run_dir), "result": records}, ensure_ascii=False))
        return 0
