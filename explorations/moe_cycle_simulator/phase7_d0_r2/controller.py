#!/usr/bin/env python3
"""Fail-closed CPU-only D0-R2 controller and terminal evidence sealer.
...
TERMINAL_MARKERS = {
    "COMPLETE": "D0_R2_COMPLETE_AUDITED\n",
    "FAILED": "D0_R2_FAILED_IMMUTABLE_NO_RETRY\n",
    "INCOMPLETE": "D0_R2_INCOMPLETE_IMMUTABLE_NO_RETRY\n",
}


class D0R2Error(RuntimeError):
    """A fail-closed D0-R2 contract or evidence error."""
...
def _validate_application_identity(root: Path, identity: dict[str, Any]) -> str:
    ...
    require(identity["schema_version"] == "moe-simulator-phase7-gputw-d0-r2-application-identity-v1", "application identity schema mismatch")
    ...
    require(identity["application_id"] == "phase7-gputw-d0-7f9804d4-20260809-r2", "application identity ID mismatch")
    return digest
...
def load_package(root: Path = PACKAGE_ROOT) -> PackageInputs:
    ...
    require(root == PACKAGE_ROOT.resolve(), "D0-R2 package must be validated from its immutable package root")
    ...
    require(manifest["schema_version"] == "moe-simulator-phase7-gputw-d0-r2-application-manifest-v1", "application manifest schema mismatch")
    require(manifest["status"] == "CPU_ONLY_DISCOVERY_PACKAGE", "application manifest status mismatch")
    require(manifest["application_id"] == "phase7-gputw-d0-7f9804d4-20260809-r2", "application manifest ID mismatch")
    ...
    validate_schema(overlay, _load_schema("d0_r2.schema.json"))
    ...
    require(overlay["promotion"]["promotable"] is False and overlay["promotion"]["execution_mode"] == "DISCOVERY_ONLY", "D0-R2 package became promotable")
    ...
    require(plan["execution_mode"] == "DISCOVERY_ONLY" and plan["retry_allowed"] is False and plan["resume_allowed"] is False, "D0-R2 plan authority changed")
...
def _validate_local_ssh(approval: dict[str, Any]) -> bytes:
...
def build_command_preimage(package: PackageInputs, approval: dict[str, Any], *, known_hosts: bytes, ssh_executable: bytes) -> dict[str, Any]:
    ...
    return {
        "schema_version": "moe-simulator-phase7-gputw-d0-r2-command-preimage-v1",
...
def _validate_approval(package: PackageInputs, approval: dict[str, Any], *, evidence_root: Path, registry_path: Path) -> tuple[bytes, bytes, bytes, dict[str, Any], str]:
    ...
    require(approval["execution_mode"] == "DISCOVERY_ONLY", "D0-R2 overlay permits discovery-only execution only")
    ...
    require(not evidence_root.exists(), "fresh evidence root already exists")
    require(not registry_path.exists(), "one-shot approval registry already exists")
    _utc_datetime(approval["lease"]["control_plane_start_utc"], "control-plane start")
    ...
    ssh_executable = _validate_local_ssh(approval)
...
def consume_one_shot_registry(approval: dict[str, Any], approval_bytes: bytes) -> tuple[bytes, str]:
    ...
    payload = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r2-used-approval-v1",
        "approval_id": approval["approval_id"],
...
def _result_identity_preimage(*, package: PackageInputs, approval: dict[str, Any], approval_bytes: bytes, registry_bytes: bytes, command_binding_sha256: str, retained_input_hashes: dict[str, str], probe_result_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": "moe-simulator-phase7-gputw-d0-r2-result-evidence-identity-preimage-v1",
        "application_id": package.overlay["application_id"],
        "application_identity_sha256": package.application_identity_sha256,
        "application_ledger_sha256": package.application_ledger_sha256,
        "reviewed_commit": approval["reviewed_commit"],
...
def _terminal_ledger(evidence_root: Path, outcome: str, *, application_identity_sha256: str, result_identity_sha256: str) -> dict[str, Any]:
    ...
    ledger: dict[str, Any] = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r2-terminal-ledger-v1",
...
def _probe_eligibility_findings(probe: dict[str, Any], approval: dict[str, Any]) -> list[str]:
    ...
    if approval["runtime"]["approved_container_digest"] is None:
        findings.append("OWNER_D0_R2_IS_DISCOVERY_ONLY")
...
def run_session(package_dir: Path, approval_path: Path, evidence_root: Path, *, runner: Runner | None = None) -> dict[str, Any]:
    package = load_package(package_dir)
    approval, approval_raw = load_json(approval_path)
    registry_path = Path(approval.get("evidence", {}).get("one_shot_registry_path", ""))
    known_hosts, ssh_executable, owner_authority_bytes, command_preimage, command_hash = _validate_approval(package, approval, evidence_root=evidence_root, registry_path=registry_path)
    registry_bytes, registry_hash = consume_one_shot_registry(approval, approval_raw)
    ...
    retained = _write_package_inputs(evidence_root, package, approval_raw, registry_bytes, owner_authority_bytes, known_hosts, ssh_executable, command_preimage)
    argv, env_overrides = build_ssh_argv(approval)
    ...
    try:
        ...
        if transport.timed_out:
            ...
            result = {
                "schema_version": "moe-simulator-phase7-gputw-d0-r2-result-v1",
            ...
            return result
    except D0R2Error as exc:
        outcome, failure_type, failure_message = "FAILED", "D0_EVIDENCE", str(exc)
        probe_result_hash = sha256_bytes(transport.stdout)
    end_utc = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    fields = _base_terminal_fields(package, approval, approval_raw, registry_bytes, command_hash, retained, probe_result_hash, start_utc, end_utc, transport.elapsed_monotonic_ns, transport.timed_out)
    failure = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r2-failure-v1",
        "terminal_status": outcome,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        require(os.environ.get("MOE_PHASE7_D0_R2_UNLOCK") == "OWNER_APPROVED_EXACT_D0_R2_COMMAND", "missing exact D0-R2 second-factor unlock")
