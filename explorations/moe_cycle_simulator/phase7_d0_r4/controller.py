#!/usr/bin/env python3
"""Fail-closed CPU-only D0-R4 controller and terminal evidence sealer.
...
TERMINAL_MARKERS = {
    "COMPLETE": "D0_R4_COMPLETE_AUDITED\n",
    "FAILED": "D0_R4_FAILED_IMMUTABLE_NO_RETRY\n",
    "INCOMPLETE": "D0_R4_INCOMPLETE_IMMUTABLE_NO_RETRY\n",
}


class D0R4Error(RuntimeError):
    """A fail-closed D0-R4 contract or evidence error."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise D0R4Error(message)
...
def _unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise D0R4Error(f"duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_float(value: str) -> None:
    raise D0R4Error(f"JSON floating-point value is forbidden: {value}")
...
def load_json_bytes(payload: bytes, label: str = "<bytes>") -> dict[str, Any]:
    try:
        ...
        raise D0R4Error(f"invalid strict JSON in {label}: {exc}") from exc
...
def canonical_bytes(value: Any) -> bytes:
    try:
        ...
        raise D0R4Error(f"value cannot be canonicalized: {exc}") from exc
...
def read_once(path: Path, *, max_bytes: int) -> bytes:
    ...
    try:
        ...
        raise D0R4Error(f"cannot open captured input {path}: {exc}") from exc
    try:
...
def write_exclusive(path: Path, payload: bytes, *, mode: int = 0o400) -> None:
    ...
    try:
        ...
        raise D0R4Error(f"refusing to overwrite {path}: {exc}") from exc
    try:
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            if key in properties:
                ...
                raise D0R4Error(f"{path}: unexpected property {key}")
...
def _validate_application_identity(root: Path, identity: dict[str, Any]) -> str:
    ...
    require(identity["schema_version"] == "moe-simulator-phase7-gputw-d0-r4-application-identity-v1", "application identity schema mismatch")
    ...
    require(identity["application_id"] == "phase7-gputw-d0-7f9804d4-20260809-r4", "application identity ID mismatch")
    return digest
...
class PackageInputs:
    ...
    application_ledger_bytes: bytes
...
def load_package(root: Path = PACKAGE_ROOT) -> PackageInputs:
    ...
    require(manifest["schema_version"] == "moe-simulator-phase7-gputw-d0-r4-application-manifest-v1", "application manifest schema mismatch")
    require(manifest["status"] == "CPU_ONLY_DISCOVERY_PACKAGE", "application manifest status mismatch")
    require(manifest["application_id"] == "phase7-gputw-d0-7f9804d4-20260809-r4", "application manifest ID mismatch")
    ...
    require(manifest["ledger_exclusions"] == ["application_ledger.sha256"], "application ledger exclusions changed")
    expected_files = set(members) | {manifest["application_ledger_path"]}
    actual_files: set[str] = set()
    expected_directories = {Path(member).parent.as_posix() for member in expected_files if Path(member).parent.as_posix() != "."}
    for entry in root.rglob("*"):
        relative = entry.relative_to(root).as_posix()
        require(not entry.is_symlink(), f"application package symlink is forbidden: {relative}")
        metadata = entry.lstat()
        if stat.S_ISREG(metadata.st_mode):
            actual_files.add(relative)
        elif stat.S_ISDIR(metadata.st_mode):
            require(relative in expected_directories, f"application package contains an extra directory: {relative}")
        else:
            raise D0R4Error(f"application package contains a special file: {relative}")
    require(actual_files == expected_files, f"application package exact-set mismatch; missing={sorted(expected_files - actual_files)}, extra={sorted(actual_files - expected_files)}")
    package_bytes: dict[str, bytes] = {}
    ...
    validate_schema(overlay, _load_schema("d0_r4.schema.json"))
    ...
    require(overlay["promotion"]["promotable"] is False and overlay["promotion"]["execution_mode"] == "DISCOVERY_ONLY", "D0-R4 package became promotable")
    ...
    require(owner_authority["cost_authority"]["maximum_runtime_seconds"] == 28800, "owner prepaid runtime changed")
    require(owner_authority["cost_authority"]["prepaid_entitlement"] == {
        "original_prepaid_seconds": 21600,
        "additional_prepaid_seconds": 7200,
        "total_prepaid_seconds": 28800,
        "status": "FULLY_PREPAID_BEFORE_R4_FREEZE",
    }, "owner prepaid entitlement changed")
    require(owner_authority["cost_authority"]["top_up"] is False and owner_authority["cost_authority"]["extension"] is False and owner_authority["cost_authority"]["extension_beyond_28800"] is False, "top-up or extension became allowed")
    require(plan["execution_mode"] == "DISCOVERY_ONLY" and plan["retry_allowed"] is False and plan["resume_allowed"] is False, "D0-R4 plan authority changed")
    require(plan["outer_timeout_seconds"] == 300 and plan["inner_probe_timeout_seconds"] == 120, "D0 timeout contract changed")
    require(plan["transport_and_cleanup_timeout_seconds"] == 280, "transport/cleanup budget changed")
    require(plan["transport_and_cleanup_timeout_seconds"] + plan["process_cleanup"]["terminal_reserve_seconds"] == plan["outer_timeout_seconds"], "outer deadline arithmetic drift")
    stage_bounds = overlay["time_contract"]["stage_bounds_seconds"]
    require(stage_bounds == {"D0": 300, "Gate_M": 5400, "M0": 14400, "release": 900}, "stage bounds changed")
    require(sum(stage_bounds.values()) == overlay["time_contract"]["bounded_work_seconds"] == 21000, "bounded work arithmetic drift")
    require(overlay["time_contract"]["total_seconds"] - overlay["time_contract"]["bounded_work_seconds"] == overlay["time_contract"]["shared_transition_slack_seconds"] == 7800, "transition slack arithmetic drift")
    require(all(overlay["time_contract"]["slack_contract"][key] is False for key in ("workload_authority", "retry_authority", "resume_authority", "timeout_extension_authority")), "slack became execution authority")
    ...
    m0_reference = load_json_bytes(package_bytes["canonical_m0_reference.json"], "canonical M0 reference")
    require(m0_reference == {
        "schema_version": "moe-simulator-phase7-d0-r4-canonical-m0-reference-v1",
        "reference_scope": "MODEL_RUNTIME_PROBE_ONLY_NOT_ALLOCATION_AUTHORITY",
        "historical_contract_path": "explorations/moe_cycle_simulator/phase7/application/m0_execution_contract.json",
        "historical_contract_file_sha256": "d2b9a9487ffe9404b441beec6d2ae8065e2b0564eeaf164a8de846b9c31734a2",
        "canonical_boundary_semantic_sha256": "3d842b4bbeceaf06a07a78f5efad42928e137b3ce8d03c44da8b661bc056884d",
        "model_revision": "eba92302a2861cdc0098cc54bc9f17cb2c47eb61",
        "precision": "BF16", "runtime": "vLLM", "quantization": None,
        "cpu_offload_gb": 0, "swap_space_gb": 0, "max_model_length": 32768,
        "input_tokens": 28672, "output_tokens": 4096, "repetitions": 3,
        "authority": "REFERENCE_ONLY_NOT_D0_GATE_M_OR_M0_AUTHORITY",
    }, "canonical M0 reference drift")
    ...
    return PackageInputs(root, overlay, owner_authority, plan, manifest, identity, application_identity_sha256, ledger_hash, ledger_bytes, package_bytes, probe_source)
...
def _utc_datetime(value: str, label: str) -> datetime:
    ...
    try:
        ...
        raise D0R4Error(f"{label} is not a timestamp") from exc
...
def build_ssh_argv(approval: dict[str, Any], *, captured_root: Path | None = None) -> tuple[list[str], dict[str, str]]:
    ...
    ssh_executable = approval["local_ssh"]["executable_path"]
    known_hosts_path = host_key["known_hosts_path"]
    identity_file: str | None = None
    if captured_root is not None:
        ssh_executable = str(captured_root / "inputs" / "local_ssh_executable")
        known_hosts_path = str(captured_root / "inputs" / "known_hosts")
        identity_file = str(captured_root / "inputs" / "selected_public_key.pub")
    remote_argv = [
        "env", f"MOE_PHASE7_CONTAINER_DIGEST={digest}",
    ...
    if identity_file is not None:
        argv.extend(["-o", f"IdentityFile={identity_file}"])
    argv.extend(["--", f"{endpoint['username']}@{endpoint['host']}", *remote_argv])
    return argv, {"SSH_AUTH_SOCK": credential["selector"]}


def _validate_known_hosts(approval: dict[str, Any]) -> tuple[bytes, bytes]:
    ...
    try:
        ...
        raise D0R4Error("known_hosts host-key blob is invalid") from exc
...
def _ssh_public_key_fingerprints(listing: bytes) -> set[str]:
    ...
    for line in listing.decode("utf-8", errors="strict").splitlines():
        ...
        try:
            ...
            raise D0R4Error("SSH agent returned an invalid public-key blob") from exc
        fingerprint = "SHA256:" + base64.b64encode(hashlib.sha256(blob).digest()).decode().rstrip("=")
...
def _select_approved_public_key(listing: bytes, approved_fingerprint: str) -> bytes:
    matches: list[bytes] = []
    for raw_line in listing.splitlines():
        fields = raw_line.split()
        require(len(fields) >= 2 and fields[0].startswith(b"ssh-"), "SSH agent returned a non-public-key line")
        try:
            blob = base64.b64decode(fields[1], validate=True)
        except (ValueError, binascii.Error) as exc:
            raise D0R4Error("SSH agent returned an invalid public-key blob") from exc
        ...
        if fingerprint == approved_fingerprint:
            matches.append(fields[0] + b" " + fields[1] + b"\n")
    require(len(matches) == 1, "approved client public-key fingerprint must match exactly one agent key")
    return matches[0]
...
def _capture_agent_listing(approval: dict[str, Any]) -> bytes:
    ...
    descriptor = os.open(str(executable), os.O_RDONLY | os.O_NOFOLLOW)
    ...
    try:
        ...
        raise D0R4Error("unable to query the approved SSH agent") from exc
    finally:
        os.close(descriptor)
...
def _validate_local_ssh(approval: dict[str, Any], *, agent_listing_provider: Callable[[dict[str, Any]], bytes] | None = None) -> tuple[bytes, bytes, bytes]:
    ...
    _ssh_public_key_fingerprints(listing)
    selected_public_key = _select_approved_public_key(listing, fingerprint)
    return raw, listing, selected_public_key


def _validate_review_closure(package: PackageInputs, approval: dict[str, Any]) -> bytes:
    descriptor = approval["review_closure_input"]
    path = Path(descriptor["path"])
    require(path.is_absolute() and not path.is_symlink() and not str(path).startswith("PENDING_"), "review closure path is not explicit")
    payload = read_once(path.resolve(strict=True), max_bytes=1024 * 1024)
    require(sha256_bytes(payload) == descriptor["sha256"], "review closure hash mismatch")
    review = load_json_bytes(payload, "R4 same-hash review authority")
    validate_schema(review, _load_schema("schemas/review_authority.schema.json"))
    require(review["blockers"] == [], "R4 same-hash review retains blockers")
    require(review["candidate_id"].startswith("moe-simulator-phase7-gputw-d0-r4-"), "review closure candidate ID is not R4")
    require(review["reviewed_commit"] == approval["reviewed_commit"], "approval reviewed commit differs from review closure")
    require(review["reviewed_tree"] == approval["reviewed_tree"], "approval reviewed tree differs from review closure")
    require(review["candidate_ledger_sha256"] == approval["reviewed_candidate_ledger_sha256"], "approval candidate ledger differs from review closure")
    require(review["application_identity_file_sha256"] == sha256_bytes(package.package_bytes["application_identity.json"]), "review closure application identity file mismatch")
    require(review["application_identity_semantic_sha256"] == package.application_identity_sha256, "review closure semantic identity mismatch")
    require(review["application_ledger_file_sha256"] == package.application_ledger_sha256, "review closure application ledger mismatch")
    return payload


def build_command_preimage(
    package: PackageInputs,
    approval: dict[str, Any],
    *,
    known_hosts: bytes,
    ssh_executable: bytes,
    review_closure: bytes = b"",
    host_provenance: bytes = b"",
    agent_listing: bytes = b"",
    selected_public_key: bytes = b"",
) -> dict[str, Any]:
    argv, env_overrides = build_ssh_argv(approval, captured_root=Path(approval["evidence"]["evidence_root"]))
    return {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-command-preimage-v1",
        ...
        "review_closure_sha256": sha256_bytes(review_closure),
        "owner_authority_sha256": approval["owner_authority_sha256"],
        "approval_id": approval["approval_id"],
        "lease_and_cost_authority": approval["lease"],
        "canonical_m0_reference_sha256": sha256_bytes(package.package_bytes["canonical_m0_reference.json"]),
        "canonical_m0_authority": "REFERENCE_ONLY_NOT_D0_GATE_M_OR_M0_AUTHORITY",
        "ssh": {
        ...
        "host_key": {
            "known_hosts_execution_path": str(Path(approval["evidence"]["evidence_root"]) / "inputs" / "known_hosts"),
            ...
            "captured_provenance_sha256": sha256_bytes(host_provenance),
        },
        "credential": {
            ...
            "agent_listing_sha256": sha256_bytes(agent_listing),
            "selected_public_key_sha256": sha256_bytes(selected_public_key),
        },
        "remote_payload_sha256": sha256_bytes(package.probe_source),
        "canonical_ssh_argv": argv,
        "outer_timeout_seconds": package.plan["outer_timeout_seconds"],
        "transport_and_cleanup_timeout_seconds": package.plan["transport_and_cleanup_timeout_seconds"],
...
def _validate_approval(
    ...
    *,
...
) -> tuple[bytes, bytes, bytes, bytes, bytes, bytes, bytes, dict[str, Any], str]:
    ...
    require(approval["execution_mode"] == "DISCOVERY_ONLY", "D0-R4 overlay permits discovery-only execution only")
    ...
    review_closure_bytes = _validate_review_closure(package, approval)
    ...
    require(deadline - lease_start == timedelta(seconds=28800), "lease window is not exactly eight prepaid hours")
    ...
    required_remaining = package.overlay["time_contract"]["bounded_work_seconds"]
    ...
    require(approval["lease"]["extension_allowed"] is False and approval["lease"]["extension_beyond_28800_allowed"] is False and approval["lease"]["top_up_allowed"] is False and approval["lease"]["provider_grace_credit_seconds"] == 0, "approval lifecycle/cost authority changed")
    require(approval["lease"]["total_seconds"] == 28800, "approval prepaid total changed")
    entitlement = approval["lease"]["prepaid_entitlement"]
    require(entitlement["original_prepaid_seconds"] + entitlement["additional_prepaid_seconds"] == entitlement["total_prepaid_seconds"] == 28800, "approval prepaid entitlement arithmetic drift")
    require(approval["lease"]["stage_bounds_seconds"] == package.overlay["time_contract"]["stage_bounds_seconds"], "approval stage bounds differ from package")
    require(sum(approval["lease"]["stage_bounds_seconds"].values()) == approval["lease"]["bounded_work_seconds"] == 21000, "approval bounded work arithmetic drift")
    require(approval["lease"]["total_seconds"] - approval["lease"]["bounded_work_seconds"] == approval["lease"]["shared_transition_slack_seconds"] == 7800, "approval slack arithmetic drift")
    require(owner_document["time_origin"]["control_plane_start_utc"] == approval["lease"]["control_plane_start_utc"], "owner/approval control-plane start mismatch")
    ...
    require(owner_document["time_origin"]["control_plane_start_utc"] == owner_document["time_origin"]["lease_start_utc"], "owner control-plane and lease starts differ")
    require(owner_document["cost_authority"]["maximum_runtime_seconds"] == approval["lease"]["total_seconds"] == 28800, "owner/approval runtime entitlement mismatch")
    require(owner_document["cost_authority"]["prepaid_entitlement"] == entitlement, "owner/approval prepaid entitlement mismatch")
    require(owner_document["cost_authority"]["maximum_additional_spend"] == {"amount": "0", "currency": "TWD"}, "owner authority spend changed")
    known_hosts, host_provenance = _validate_known_hosts(approval)
    ssh_executable, agent_listing, selected_public_key = _validate_local_ssh(approval, agent_listing_provider=agent_listing_provider)
    ...
    command_preimage = build_command_preimage(
        package, approval, known_hosts=known_hosts, ssh_executable=ssh_executable,
        review_closure=review_closure_bytes, host_provenance=host_provenance,
        agent_listing=agent_listing, selected_public_key=selected_public_key,
    )
    command_hash = semantic_sha256(command_preimage)
    require(approval["command_binding_sha256"] == command_hash, "COMMAND_PREIMAGE_DRIFT")
    return known_hosts, ssh_executable, owner_bytes, review_closure_bytes, host_provenance, agent_listing, selected_public_key, command_preimage, command_hash


def one_shot_registry_bytes(approval: dict[str, Any], approval_bytes: bytes) -> bytes:
    payload = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-used-approval-v1",
        "approval_id": approval["approval_id"],
...
def _enable_child_subreaper() -> None:
    """Make orphaned double-fork/setsid descendants reparent to this controller."""

    libc = ctypes.CDLL(None, use_errno=True)
    observed = ctypes.c_int(0)
    if libc.prctl(36, 1, 0, 0, 0) != 0:
        raise D0R4Error(f"cannot enable child subreaper: errno={ctypes.get_errno()}")
    if libc.prctl(37, ctypes.byref(observed), 0, 0, 0) != 0 or observed.value != 1:
        raise D0R4Error("child subreaper verification failed")
...
def _terminate_tree(
    ...
    *,
    term_grace: float,
    kill_grace: float,
    containment_root_pid: int | None = None,
    ...
    baseline = set() if baseline_pids is None else set(baseline_pids)

    def contained() -> set[int]:
        values = _descendants(process.pid)
        if containment_root_pid is not None:
            values.update(_descendants(containment_root_pid) - baseline)
    ...
    if absolute_deadline is not None:
        term_deadline = min(term_deadline, absolute_deadline)
    while time.monotonic() < term_deadline and process.poll() is None:
        known.update(contained())
        time.sleep(0.01)
    ...
    if absolute_deadline is not None:
        kill_deadline = min(kill_deadline, absolute_deadline)
    try:
        process.wait(timeout=max(0.001, kill_deadline - time.monotonic()))
    except subprocess.TimeoutExpired as exc:
        raise D0R4Error("process tree survived kill boundary") from exc
...
def run_bounded_process(argv: list[str], payload: bytes, *, env_overrides: dict[str, str], timeout_seconds: int, max_stdout_bytes: int, max_stderr_bytes: int) -> TransportResult:
    ...
    containment_root = os.getpid()
    baseline_pids = _descendants(containment_root)
    environment = {key: os.environ[key] for key in ("PATH", "LANG", "LC_ALL", "TZ") if key in os.environ}
    ...
    absolute_deadline = started / 1_000_000_000 + timeout_seconds
    cleanup_reserve = min(5.0, max(0.1, timeout_seconds / 5.0))
    transport_deadline = absolute_deadline - cleanup_reserve
    try:
        while selector.get_map():
            remaining = transport_deadline - time.monotonic()
            if remaining <= 0:
                timed_out = True
                _terminate_tree(process, term_grace=2, kill_grace=2, containment_root_pid=containment_root, baseline_pids=baseline_pids, absolute_deadline=absolute_deadline)
                break
            ...
            if timed_out or output_limited:
                break
        escaped = _descendants(containment_root) - baseline_pids
        if process.poll() is None or escaped:
            _terminate_tree(process, term_grace=2, kill_grace=2, containment_root_pid=containment_root, baseline_pids=baseline_pids, absolute_deadline=absolute_deadline)
    ...
    if elapsed > timeout_seconds * 1_000_000_000:
        timed_out = True
    return TransportResult(returncode, bytes(streams[stdout_fd]), bytes(streams[stderr_fd]), timed_out, output_limited, elapsed)
...
def _result_identity_preimage(*, package: PackageInputs, approval: dict[str, Any], approval_bytes: bytes, registry_bytes: bytes, command_binding_sha256: str, retained_input_hashes: dict[str, str], probe_result_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-result-evidence-identity-preimage-v1",
        "application_id": package.overlay["application_id"],
        "application_identity_sha256": package.application_identity_sha256,
        "application_ledger_sha256": package.application_ledger_sha256,
        "reviewed_commit": approval["reviewed_commit"],
...
def _terminal_ledger(evidence_root: Path, outcome: str, *, application_identity_sha256: str, result_identity_sha256: str) -> dict[str, Any]:
    ...
    for path in sorted(evidence_root.rglob("*")):
        if path.is_symlink():
            raise D0R4Error(f"terminal evidence symlink is forbidden: {path}")
        if path.is_dir():
            continue
    ...
    ledger: dict[str, Any] = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-terminal-ledger-v1",
...
def verify_terminal(evidence_root: Path) -> dict[str, Any]:
    ...
    for path in evidence_root.rglob("*"):
        relative = path.relative_to(evidence_root).as_posix()
        require(not path.is_symlink(), f"terminal evidence symlink is forbidden: {relative}")
        metadata = path.lstat()
        if stat.S_ISDIR(metadata.st_mode):
            require(metadata.st_mode & 0o222 == 0, f"terminal evidence directory remains writable: {relative}")
        elif stat.S_ISREG(metadata.st_mode):
            if path.name != "terminal_ledger.json":
                actual.add(relative)
        else:
            raise D0R4Error(f"terminal evidence special file is forbidden: {relative}")
    ...
    require(evidence_root.stat().st_mode & 0o222 == 0, "terminal evidence root remains writable")

    result_path = evidence_root / "d0_result.json"
    failure_path = evidence_root / "d0_failure.json"
    if ledger["terminal_status"] == "COMPLETE":
        require(result_path.is_file() and not failure_path.exists(), "COMPLETE terminal must contain exactly one result record")
        record, _ = load_json(result_path)
        validate_schema(record, _load_schema("schemas/result.schema.json"))
    else:
        require(failure_path.is_file() and not result_path.exists(), "non-COMPLETE terminal must contain exactly one failure record")
        record, _ = load_json(failure_path)
        validate_schema(record, _load_schema("schemas/failure.schema.json"))
    require(record["terminal_status"] == ledger["terminal_status"], "record and terminal status differ")
    require(record["application_identity_sha256"] == ledger["application_identity_sha256"], "terminal application identity differs from record")
    require(record["result_evidence_identity_sha256"] == ledger["result_evidence_identity_sha256"], "terminal result identity differs from record")

    retained = record["retained_input_hashes"]
    for relative, expected in retained.items():
        retained_path = evidence_root / relative
        require(retained_path.is_file() and not retained_path.is_symlink(), f"retained input missing: {relative}")
        require(file_sha256(retained_path) == expected, f"retained input hash drift: {relative}")
    approval_bytes = read_once(evidence_root / "inputs/approval.json", max_bytes=1024 * 1024)
    approval = load_json_bytes(approval_bytes, "retained approval")
    registry_bytes = read_once(evidence_root / "inputs/one_shot_registry.json", max_bytes=1024 * 1024)
    command_preimage = load_json_bytes(read_once(evidence_root / "inputs/command_preimage.json", max_bytes=4 * 1024 * 1024), "retained command preimage")
    review_authority = load_json_bytes(read_once(evidence_root / "inputs/review_authority.json", max_bytes=1024 * 1024), "retained review authority")
    identity = load_json_bytes(read_once(evidence_root / "inputs/package/application_identity.json", max_bytes=1024 * 1024), "retained application identity")
    identity_semantic = semantic_sha256(identity["identity_preimage"])
    require(identity_semantic == identity["application_identity_sha256"] == record["application_identity_sha256"], "retained application semantic identity mismatch")
    application_ledger_bytes = read_once(evidence_root / "inputs/package/application_ledger.sha256", max_bytes=4 * 1024 * 1024)
    require(sha256_bytes(application_ledger_bytes) == record["application_ledger_sha256"], "retained application ledger identity mismatch")
    require(semantic_sha256(command_preimage) == record["command_binding_sha256"], "retained command preimage mismatch")
    require(sha256_bytes(approval_bytes) == record["owner_approval_sha256"], "retained owner approval mismatch")
    require(sha256_bytes(registry_bytes) == record["one_shot_registry_sha256"], "retained one-shot registry mismatch")
    require(review_authority["reviewed_commit"] == record["reviewed_commit"] and review_authority["reviewed_tree"] == record["reviewed_tree"], "retained review authority identity mismatch")
    require(sha256_bytes(read_once(evidence_root / "inputs/probe_source.py", max_bytes=32 * 1024 * 1024)) == record["probe_input_sha256"], "retained probe input mismatch")
    require(sha256_bytes(read_once(evidence_root / "transport/stdout.bin", max_bytes=2 * 1024 * 1024)) == record["probe_result_sha256"], "retained probe result mismatch")
    identity_preimage = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-result-evidence-identity-preimage-v1",
        ...
        "command_binding_sha256": record["command_binding_sha256"],
        "owner_authority_sha256": record["owner_authority_sha256"],
        "owner_approval_sha256": record["owner_approval_sha256"],
        "session_id": record["session_id"],
        "one_shot_registry_sha256": record["one_shot_registry_sha256"],
        "retained_input_hashes": dict(sorted(retained.items())),
        "probe_input_sha256": record["probe_input_sha256"],
        "probe_result_sha256": record["probe_result_sha256"],
    }
    require(semantic_sha256(identity_preimage) == record["result_evidence_identity_sha256"], "result evidence semantic identity mismatch")
    return ledger
...
def _publish_terminal(evidence_root: Path, outcome: str, *, application_identity_sha256: str, result_identity_sha256: str) -> dict[str, Any]:
    ...
    for staged, final in (("terminal_ledger.json.staged", "terminal_ledger.json"), ("terminal_status.txt.staged", "terminal_status.txt")):
        try:
            ...
            raise D0R4Error(f"terminal artifact already exists: {final}") from exc
...
def _write_package_inputs(
    ...
    owner_authority_bytes: bytes,
    review_closure_bytes: bytes,
    known_hosts: bytes,
    ssh_executable: bytes,
    host_provenance_bytes: bytes,
    agent_listing: bytes,
    selected_public_key: bytes,
    command_preimage: dict[str, Any],
) -> dict[str, str]:
    hashes: dict[str, str] = {}
    ...
    package_ledger_target = evidence_root / "inputs" / "package" / "application_ledger.sha256"
    write_exclusive(package_ledger_target, package.application_ledger_bytes)
    hashes[package_ledger_target.relative_to(evidence_root).as_posix()] = sha256_bytes(package.application_ledger_bytes)
    static_inputs = {
        ...
        "inputs/review_authority.json": review_closure_bytes,
        "inputs/known_hosts": known_hosts,
        "inputs/host_key_provenance.json": host_provenance_bytes,
        "inputs/agent_listing.stdout": agent_listing,
        "inputs/selected_public_key.pub": selected_public_key,
        "inputs/local_ssh_identity.json": canonical_bytes({"execution_path": "inputs/local_ssh_executable", "sha256": sha256_bytes(ssh_executable), "private_key_material": "FORBIDDEN_NOT_RECORDED"}),
    ...
    ssh_target = evidence_root / "inputs" / "local_ssh_executable"
    write_exclusive(ssh_target, ssh_executable, mode=0o500)
    hashes["inputs/local_ssh_executable"] = sha256_bytes(ssh_executable)
...
def _probe_eligibility_findings(probe: dict[str, Any], approval: dict[str, Any]) -> list[str]:
    ...
    if approval["runtime"]["approved_container_digest"] is None:
        findings.append("OWNER_D0_R4_IS_DISCOVERY_ONLY")
...
def run_session(
    ...
    *,
    ...
    (
        known_hosts,
        ssh_executable,
        owner_authority_bytes,
        review_closure_bytes,
        host_provenance_bytes,
        agent_listing,
        selected_public_key,
        command_preimage,
        command_hash,
    ...
    retained = _write_package_inputs(
        evidence_root, package, approval_raw, planned_registry_bytes, owner_authority_bytes,
        review_closure_bytes, known_hosts, ssh_executable, host_provenance_bytes,
        agent_listing, selected_public_key, command_preimage,
    )
    argv, env_overrides = build_ssh_argv(approval, captured_root=evidence_root)
    ...
    try:
        ...
        transport = transport_runner(argv, package.probe_source, env_overrides, package.plan["transport_and_cleanup_timeout_seconds"], package.plan["max_stdout_bytes"], package.plan["max_stderr_bytes"])
        ...
        if transport.timed_out:
            outcome, failure_type, failure_message = "INCOMPLETE", "D0_OUTER_TIMEOUT", "bounded D0 transport and cleanup exhausted its 280-second budget inside the 300-second outer envelope"
            ...
            result = {
                "schema_version": "moe-simulator-phase7-gputw-d0-r4-result-v1",
            ...
            return result
    except (D0R4Error, OSError) as exc:
    ...
    fields = _base_terminal_fields(package, approval, approval_raw, registry_bytes, command_hash, retained, probe_result_hash, start_utc, end_utc, transport.elapsed_monotonic_ns, transport.timed_out)
    failure = {
        "schema_version": "moe-simulator-phase7-gputw-d0-r4-failure-v1",
        "terminal_status": outcome,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        require(os.environ.get("MOE_PHASE7_D0_R4_UNLOCK") == "OWNER_APPROVED_EXACT_D0_R4_COMMAND", "missing exact D0-R4 second-factor unlock")
        result = run_session(args.package_dir, args.approval, args.evidence_root)
        print(json.dumps(result, ensure_ascii=False, sort_keys=True))
        return 0
    except (D0R4Error, OSError, ValueError) as exc:
