#!/usr/bin/env python3
"""Build, test and seal the CPU-only Phase 4 suite."""
...
from validate_phase4 import source_root, validate_source  # noqa: E402
...
def execute(command: list[str], environment: dict[str, str] | None = None):
...
def log(logs: Path, name: str, command: list[str], result) -> None:
    write_json(
...
def run(output: Path, command: list[str]) -> bool:
    if output.exists():
        raise RuntimeError("fresh Phase 4 run already exists")
    validate_source()
    logs = output / "logs"
    environment = output / "environment"
    logs.mkdir(parents=True)
    environment.mkdir()
    normal = Path(tempfile.mkdtemp(prefix="moe-phase4-build-"))
    sanitizer = Path(tempfile.mkdtemp(prefix="moe-phase4-asan-"))
    results = []
    try:
        commands = [
            ["cmake", "-S", str(PHASE4_ROOT), "-B", str(normal),
             "-DCMAKE_BUILD_TYPE=Release"],
            ["cmake", "--build", str(normal), "--parallel", "2"],
            ["ctest", "--test-dir", str(normal), "--output-on-failure"],
            ["cmake", "-S", str(PHASE4_ROOT), "-B", str(sanitizer),
             "-DCMAKE_BUILD_TYPE=Debug",
             "-DCMAKE_CXX_FLAGS=-fsanitize=address,undefined -fno-omit-frame-pointer",
             "-DCMAKE_EXE_LINKER_FLAGS=-fsanitize=address,undefined"],
            ["cmake", "--build", str(sanitizer), "--parallel", "2"],
        ]
        names = ["configure", "build", "ctest", "asan_configure", "asan_build"]
        ...
        if len(results) == 5 and all(value[2][0] == 0 for value in results):
            for name, binary in [
                ("asan_phase4", sanitizer / "moe_sim_phase4_tests"),
                ("asan_phase3", sanitizer / "phase3_core/moe_sim_phase3_tests"),
            ]:
                item = [str(binary)]
                results.append((name, item, execute(item, asan_env)))
        ...
        write_json(
            ...
            {
                "run_id": output.name,
                "experiment_id": "moe_cycle_simulator_phase4_single_gpu",
                "stage": "S4",
        ...
        write_json(
            output / "metrics.json",
            {
                ...
                "source_root_sha256": source_root(),
                "ctest_passed": 3 if passed else 0,
...
def main() -> int:
    ...
    print("PHASE4_SINGLE_GPU_SUITE:", "PASS" if passed else "FAIL")
