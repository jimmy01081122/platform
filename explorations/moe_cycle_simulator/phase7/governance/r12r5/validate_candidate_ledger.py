#!/usr/bin/env python3
"""Validate the Phase 7 R12-R5 candidate ledger as an exact file set."""
...
PHASE7 = GOVERNANCE.parents[1]
REPO = PHASE7.parents[2]
LEDGER = GOVERNANCE / "checksums.sha256"
ACTIVE_LEDGER = LEDGER.relative_to(REPO).as_posix()
REVIEW_PREFIX = "explorations/moe_cycle_simulator/phase7/governance/r12r5/reviews/"
RUN = REPO / (
    "runs/20260801T183625Z__"
    "moe_cycle_simulator_phase7_cpu_governance_r12r5_final__S2"
)
FIXED = {
    ...
    "explorations/moe_cycle_simulator/phase6/governance/reviews/phase6_r4_checksums.sha256",
    "explorations/moe_cycle_simulator/phase6/governance/reviews/phase6_r4_aggregate.json",
...
def main() -> int:
    ...
    if set(listed) != expected:
        raise RuntimeError(json.dumps({"missing": sorted(expected - set(listed)), "extra": sorted(set(listed) - expected)}, sort_keys=True))
    ...
    print(json.dumps({"status": "PASS", "member_count": len(listed), "ledger_sha256": digest(LEDGER), "nested_checksum_ledgers": "INCLUDED", "gpu_authority": "NONE"}, indent=2, sort_keys=True))
    return 0
