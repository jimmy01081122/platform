...
def add_overlap_point(point: dict[str, Any], fn_factory: Callable[[], tuple[Callable[[], dict[str, Any]], Callable[[], None]]], output_dir: Path, repetitions: int = 10) -> None:
    ...
    try:
        fn()
        torch.cuda.synchronize()
        def profile_overlap() -> int:
            result = fn()
            return int(result.get("requested_bytes", point.get("requested_bytes", 0)))

        profiler = profiler_canary(profile_overlap)
        samples = []
        ...
        point = {**point, "status": "PASS", "measurement_class": "GPU_TRANSFER_PROBE", "measured_repetition_count": len(samples), "wall_gpu_duration_ns": summarize([sample["wall_gpu_duration_ns"] for sample in samples]), "samples": samples, "profiler_canary": profiler}
...
def main() -> int:
    ...
    for layer in range(int(config["num_hidden_layers"])):
        for expert in range(int(config["num_local_experts"])):
            ...
            tensors = []
            for source in object_sources(sources, layer, expert):
                payload_bytes = int(source["bytes"])
                tensors.append({
                    "layer_id": layer,
                    "local_expert_id": expert,
                    "global_object_id": object_id,
                    "tensor_name": source["name"].split(f".experts.{expert}.", 1)[1],
                    **source,
                    "materialized_bytes": payload_bytes,
                    "packed_bytes": payload_bytes,
                    "aligned_bytes": align_up(payload_bytes, RUNTIME_ALIGNMENT_BYTES),
                    "alignment_bytes": RUNTIME_ALIGNMENT_BYTES,
                })
            catalog[str(object_id)] = {
                "layer_id": layer,
                "local_expert_id": expert,
                "global_object_id": object_id,
                "object_granularity": "layer_expert_three_tensor_bundle",
                "ownership": {
                    "checkpoint_owner": "immutable_model_checkpoint",
                    "host_owner": "transfer_probe_process",
                    "device_owner": "transfer_probe_process:cuda:0",
                },
                "mutable": False,
                "writeback_allowed": False,
                "eviction_semantics": "RELEASE_DISCARD_AND_RELOAD",
                "materialized_bytes": sum(int(tensor["materialized_bytes"]) for tensor in tensors),
                "packed_bytes": sum(int(tensor["packed_bytes"]) for tensor in tensors),
                "aligned_bytes": sum(int(tensor["aligned_bytes"]) for tensor in tensors),
                "alignment_bytes": RUNTIME_ALIGNMENT_BYTES,
                "tensors": tensors,
            }
    write_json(args.output_dir / "expert_catalog.json", {
        "schema_version": "phase7-expert-catalog-v2",
        "model_revision": MODEL_REVISION,
        "identity_formula": "layer_id * num_local_experts + local_expert_id",
        "object_count": len(catalog),
        "objects": catalog,
        "packed_bytes_semantics": "uncompressed safetensors tensor payload; container header and inter-tensor padding excluded",
        "aligned_bytes_semantics": "sum of tensor payloads rounded independently to the explicit transfer-descriptor alignment; allocator reservation excluded",
        "immutable_weight_eviction_semantics": "release/discard device allocation and reload from immutable checkpoint-backed host source; no D2H writeback",
        "content_identity_note": "checkpoint revision, tensor name, shard, dtype, shape and known bytes; no full checkpoint hash rescan",
    })
    write_json(args.output_dir / "manifest.json", {"schema_version": "phase7-transfer-probe-v2", "status": "RUNNING", "model_path": str(args.model_dir), "model_revision": MODEL_REVISION, "config_dimensions": {key: config.get(key) for key in ("num_hidden_layers", "num_local_experts", "num_experts_per_tok", "hidden_size", "intermediate_size")}, "object_bytes_E": object_bytes, "object_bytes_E_semantics": "materialized tensor payload bytes", "runtime_catalog_schema": "phase7-expert-catalog-v2", "object_tensor_names": [source["name"] for source in expert0], "groups": [item.strip() for item in args.groups.split(",") if item.strip()], "repetitions": args.repetitions, "max_repetitions": args.max_repetitions, "measurement_class": "GPU_TRANSFER_PROBE", "raw_model_hash_not_rescanned": True})
