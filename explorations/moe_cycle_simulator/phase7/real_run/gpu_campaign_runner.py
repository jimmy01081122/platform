...
SAMPLING_MODES = {
    "FORCED_LENGTH_CONTROLLED",
    "FORCED_LENGTH_NATURAL_FIXTURE",
    "NATURAL_EOS_CAPPED",
}
RUNTIME_CLASSES = {
    "CLEAN",
...
def sampling_params(vllm_module: Any, output_tokens: int, sampling_mode: str) -> tuple[Any, dict[str, Any]]:
    if output_tokens <= 0:
    ...
    params: dict[str, Any] = dict(
        ...
        top_k=SAMPLING_BASE["top_k"],
        seed=SAMPLING_BASE["seed"],
        stop=SAMPLING_BASE["stop"],
        max_tokens=output_tokens,
        detokenize=True,
        skip_special_tokens=False,
    )
    if sampling_mode == "NATURAL_EOS_CAPPED":
        params["ignore_eos"] = False
    elif sampling_mode in {"FORCED_LENGTH_CONTROLLED", "FORCED_LENGTH_NATURAL_FIXTURE"}:
        params["ignore_eos"] = True
        params["min_tokens"] = output_tokens
    else:
        raise CampaignFailure(f"unsupported sampling mode: {sampling_mode}")
    return vllm_module.SamplingParams(**params), params


def runtime_constructor_args(
    runtime_class: str,
    max_num_seqs: int,
    ...
    if runtime_class in {"ROUTING", "ROUTING_PROFILE"}:
...
def run_one_request(
    ...
    if sampler is not None:
        sampler.start()
    params, actual_sampling_params = sampling_params(vllm_module, output_tokens, sampling_mode)
    ...
    try:
        ...
        if runtime_class in {"KERNEL_PROFILE", "ROUTING_PROFILE"}:
    ...
    record: dict[str, Any] = {
        ...
        "sampling": actual_sampling_params,
    }
    ...
    elif sampling_mode == "NATURAL_EOS_CAPPED":
        if not (0 < actual_output_count <= output_tokens):
            raise CampaignFailure(
                f"natural-EOS output length is outside cap for {logical_id}: "
                f"actual={actual_output_count}, cap={output_tokens}"
            )
        if getattr(completion, "finish_reason", None) not in {"stop", "length"}:
            raise CampaignFailure(
                f"natural-EOS request has unexpected finish reason for {logical_id}: "
                f"{getattr(completion, 'finish_reason', None)!r}"
            )
    if runtime_class in {"ROUTING", "ROUTING_PROFILE"}:
...
def load_engine(
    ...
    if runtime_class in {"KERNEL_PROFILE", "ROUTING_PROFILE"}:
        ...
        args["profiler_config"] = {
            ...
            "warmup_iterations": 0,
            "active_iterations": profiler_max_iterations if runtime_class == "ROUTING_PROFILE" else 5,
            "wait_iterations": 0,
        }
    ...
    write_json(run_dir / "requested_engine_args.json", json_safe(args))
    llm = llm_class(**args)
    resolved = resolved_runtime_record(llm, args, signature)
    write_json(run_dir / "resolved_runtime.json", json_safe(resolved))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sampling-mode", choices=sorted(SAMPLING_MODES), default="FORCED_LENGTH_CONTROLLED")
    parser.add_argument("--sampling-pair-id")
    parser.add_argument("--sampling-pair-role", choices=("standalone", "clean", "instrumented"), default="standalone")
    ...
    parser.add_argument("--cpu-offload-param", action="append", default=[])
    parser.add_argument("--offload-backend", choices=("auto", "uva", "prefetch"), default="auto")
...
def main() -> int:
    ...
    try:
        ...
        if args.cpu_offload_gb < 0:
            raise CampaignFailure("cpu-offload-gb must be >= 0")
        ...
        if args.sampling_pair_role == "standalone" and args.sampling_pair_id is not None:
            raise CampaignFailure("standalone sampling runs must not set --sampling-pair-id")
        if args.sampling_pair_role != "standalone" and not args.sampling_pair_id:
            raise CampaignFailure("paired sampling runs require --sampling-pair-id")
        if args.sampling_pair_role == "clean" and args.runtime_class != "CLEAN":
            raise CampaignFailure("sampling pair role clean requires runtime class CLEAN")
        if args.sampling_pair_role == "instrumented" and args.runtime_class == "CLEAN":
            raise CampaignFailure("sampling pair role instrumented requires a non-CLEAN runtime class")
        if args.sampling_mode == "NATURAL_EOS_CAPPED" and args.request_plan is not None:
            raise CampaignFailure("natural-EOS request-plan mode is not implemented in this runner revision")
        exact_output_required = args.sampling_mode != "NATURAL_EOS_CAPPED" and not args.no_exact_output
        ...
        write_json(
            run_dir / "manifest.json",
            {
                ...
                "sampling_pair_id": args.sampling_pair_id,
                "sampling_pair_role": args.sampling_pair_role,
                "sampling": {
                    **SAMPLING_BASE,
                    "ignore_eos": args.sampling_mode != "NATURAL_EOS_CAPPED",
                    "min_tokens": None if args.sampling_mode == "NATURAL_EOS_CAPPED" else args.output_tokens,
                ...
                "cpu_offload_params": sorted(set(args.cpu_offload_param)),
        ...
        if args.request_plan is not None:
            ...
            for sequence_index in range(1, request_plan["repeat_count"] + 1):
                ...
                for sequence_position, plan_request in enumerate(materialized_plan_requests, start=1):
                    ...
                    record = run_one_request(
                        ...
                        exact_output_tokens=plan_request["output_tokens"] if exact_output_required else None,
                        sampling_mode=args.sampling_mode,
            ...
            for repetition_index in range(total_repetitions):
                ...
                record = run_one_request(
                    ...
                    exact_output_tokens=args.output_tokens if exact_output_required else None,
                    sampling_mode=args.sampling_mode,
