#!/usr/bin/env python3
"""Validate the prospective D0-R3 CPU-only package without network or SSH."""
...
from explorations.moe_cycle_simulator.phase7_d0_r3.controller import (  # noqa: E402
    D0R3Error,
    PACKAGE_ROOT,
    load_package,
    validate_schema,
...
def validate_package() -> dict[str, object]:
    ...
    for schema_name in (
        ...
        if not isinstance(schema, dict) or not schema.get("$id"):
            raise D0R3Error(f"schema is not a complete JSON Schema document: {schema_name}")
    return {
        "status": "PASS",
        ...
        "blockers": [
            ...
            "D0_R3_SAME_HASH_REVIEW_PENDING",
        ],
        "network_free": True,
        "ssh_attempted": False,
        "gpu_queried": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.package_dir.resolve() != PACKAGE_ROOT.resolve():
            raise D0R3Error("this validator only accepts the prospective D0-R3 package root")
        result = validate_package()
    except (D0R3Error, OSError, KeyError, TypeError, ValueError) as exc:
