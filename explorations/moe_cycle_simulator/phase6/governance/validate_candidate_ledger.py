#!/usr/bin/env python3
"""Validate the Phase 6 R4 candidate ledger as an exact file-set."""
...
PHASE6 = GOVERNANCE.parent
REPO = PHASE6.parents[2]
LEDGER = GOVERNANCE / "checksums.sha256"
ACTIVE_LEDGER = LEDGER.relative_to(REPO).as_posix()
R4_REVIEW_PREFIX = (
    "explorations/moe_cycle_simulator/phase6/governance/reviews/phase6_r4_"
)
RUN = REPO / (
    "runs/20260729T105347Z__"
    "moe_cycle_simulator_phase6_multi_domain_r4__S4"
)
FIXED = {
    "experiments/specs/moe_cycle_simulator_phase6.yaml",
    "explorations/moe_cycle_simulator/phase5/governance/checksums.sha256",
    (
        "explorations/moe_cycle_simulator/phase5/governance/reviews/"
        "phase5_r2_checksums.sha256"
    ),
    (
        "explorations/moe_cycle_simulator/phase5/governance/reviews/"
        "phase5_r2_aggregate.json"
    ),
}
...
def main() -> int:
    expected = FIXED | regular_files(PHASE6) | regular_files(RUN)
