#!/usr/bin/env python3
"""Build, test and seal the CPU-only Phase 6 suite."""
...
from validate_phase6 import source_root, validate_source  # noqa: E402
...
def run(output: Path, command: list[str]) -> bool:
    if output.exists():
        raise RuntimeError("fresh Phase 6 run already exists")
    validate_source()
    logs = output / "logs"
    environment = output / "environment"
    logs.mkdir(parents=True)
    environment.mkdir()
    normal = Path(tempfile.mkdtemp(prefix="moe-phase6-build-"))
    sanitizer = Path(tempfile.mkdtemp(prefix="moe-phase6-asan-"))
    ...
    try:
        ...
        if len(results) == 5 and all(value[2][0] == 0 for value in results):
            for name, binary in [
                ("asan_phase6", sanitizer / "moe_sim_phase6_tests"),
                ("asan_phase5", sanitizer / "phase5_policy/moe_sim_phase5_tests"),
                (
                    "asan_phase4",
                    sanitizer / "phase5_policy/phase4_model/moe_sim_phase4_tests",
                ),
                (
                    "asan_phase3",
                    sanitizer
                    / "phase5_policy/phase4_model/phase3_core/"
                    "moe_sim_phase3_tests",
                ),
            ]:
                item = [str(binary)]
                results.append(
        ...
        write_json(
            output / "manifest.json",
            {
                "run_id": output.name,
                "experiment_id": (
                    "moe_cycle_simulator_phase6_multi_gpu_coherent_uma"
                ),
                "stage": "S4",
        ...
        write_json(
            output / "metrics.json",
            {
                ...
                "source_root_sha256": source_root(),
                "ctest_passed": 5 if passed else 0,
...
def main() -> int:
    ...
    print("PHASE6_MULTI_DOMAIN_SUITE:", "PASS" if passed else "FAIL")
