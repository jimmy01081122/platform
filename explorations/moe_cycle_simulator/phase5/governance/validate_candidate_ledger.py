#!/usr/bin/env python3
"""Validate the Phase 5 R2 candidate ledger as an exact file-set."""
...
PHASE5 = GOVERNANCE.parent
REPO = PHASE5.parents[2]
LEDGER = GOVERNANCE / "checksums.sha256"
ACTIVE_LEDGER = LEDGER.relative_to(REPO).as_posix()
R2_REVIEW_PREFIX = (
    "explorations/moe_cycle_simulator/phase5/governance/reviews/phase5_r2_"
)
RUN = REPO / (
    "runs/20260728T130835Z__"
    "moe_cycle_simulator_phase5_routing_residency_r2__S4"
)
FIXED = {
    "experiments/specs/moe_cycle_simulator_phase5.yaml",
    "explorations/moe_cycle_simulator/phase4/governance/checksums.sha256",
    (
        "explorations/moe_cycle_simulator/phase4/governance/reviews/"
        "phase4_r4_aggregate.json"
    ),
}
...
def main() -> int:
    expected = FIXED | regular_files(PHASE5) | regular_files(RUN)
    expected.discard(ACTIVE_LEDGER)
    expected = {
        path for path in expected if not path.startswith(R2_REVIEW_PREFIX)
    }
    listed: dict[str, str] = {}
